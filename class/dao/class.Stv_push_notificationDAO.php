<?php
		/**
 * Classe que representa os comandos de acesso aos dados da tabela "Stv_push_notification" 
 */
class Stv_push_notificationDAO extends PDO
{
	/**
	...
	/**
	 * metodo que faz a insercao de um registro na tabela Stv_push_notification
	 */
	public function insert($obj) {
		//pegar os dados do objeto
		$campanha = $this->dba->quote($obj->getCampanha());
		$titulo = $this->dba->quote($obj->getTitulo());
		$texto = $this->dba->quote($obj->getTexto());
		$status_notificacao_interna = $this->dba->quote($obj->getStatus_notificacao_interna());
		...
		$programacao_envio = $this->dba->quote($obj->getProgramacao_envio());
		$data_hora_agendamento = $this->dba->quote($obj->getData_hora_agendamento());
		$data_hora_envio = $this->dba->quote($obj->getData_hora_envio());
		...
		$sql = "insert into stv_push_notification 
				(
				campanha,
				titulo,
				texto,
				status_notificacao_interna,
				data_registro,
				ip_registro,
				usuario_registro,
				status,
	...
	/**
	 * metodo que faz a atualizacao de um registro na tabela Stv_push_notification
	 * para remover o valor de uma coluna, atribua a string '!NULL!'
	 */
	public function update($obj) {
		//pegar os dados do objeto
		$id = $obj->getId();
		$campanha = $obj->getCampanha();
		$titulo = $obj->getTitulo();
		$texto = $obj->getTexto();
		$status_notificacao_interna = $obj->getStatus_notificacao_interna();
		$data_registro = $obj->getData_registro();
		$ip_registro = $obj->getIp_registro();
		$usuario_registro = $obj->getUsuario_registro();
		$status = $obj->getStatus();
		$programacao_envio = $obj->getProgramacao_envio();
		$data_hora_agendamento = $obj->getData_hora_agendamento();
		$data_hora_envio = $obj->getData_hora_envio();
        ...
        $campos = '';
        if (!empty($campanha) || is_numeric($campanha)) {
        	$campos .= "campanha=".$this->dba->quote($campanha).", ";
        }
        ...
        if (!empty($status_notificacao_interna) || is_numeric($status_notificacao_interna)) {
        	$campos .= "status_notificacao_interna=".$this->dba->quote($status_notificacao_interna).", ";
        }
        ...
        if (!empty($programacao_envio) || is_numeric($programacao_envio)) {
        	$campos .= "programacao_envio=".$this->dba->quote($programacao_envio).", ";
        }
        if (!empty($data_hora_agendamento) || is_numeric($data_hora_agendamento)) {
        	$campos .= "data_hora_agendamento=".$this->dba->quote($data_hora_agendamento).", ";
        }
        if (!empty($data_hora_envio) || is_numeric($data_hora_envio)) {
        	$campos .= "data_hora_envio=".$this->dba->quote($data_hora_envio).", ";
        }
		...
		//montar o comando SQL
        $sql = "update stv_push_notification set
				$campos
				where id = $id";
	...
	/**
	 * metodo que faz a exclusao de um registro na tabela Stv_push_notification
	 */
	public function delete($obj) {
		...
		//montar o comando SQL
		$sql = "delete from stv_push_notification 
				where id = $id";
	...
	/**
	 * metodo que retorna os regsitros da tabela Stv_push_notification conforme o filtro informado
	...
	public function select($where='', $order='') {
		...
		$sql = "select * from stv_push_notification $where $order";
    ...
    /**
	 * metodo que retorna o ultimo id cadastrado na tabela Stv_push_notification
	 */
    ...
    public function metaValues($coluna, $primeiro = NULL, $excluir = NULL) {
		...
		$res = $this->dba->query("SHOW COLUMNS FROM stv_push_notification WHERE Field = '".$coluna."'");
